...
export default function UserDropdown() {
  ...
  const [fetchUserData, setFetchUserData] = useState<fetchUserData | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("❌ UserDropdown - Failed to fetch user data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const firstName = fetchUserData?.firstName || "User";
      ...
      >
        {isLoading ? (
          <div className="flex items-center space-x-2">
            <Spin size="small" className="pr-4" />
          ...
          <>
            <div>
              ...
              <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">
                {fetchUserData?.email || "No email provided"}
              </span>
            </div>
